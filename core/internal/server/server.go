...
func (s *Server) Serve(printDocs bool) error {
	...
	if printDocs {
		...
		log.Info(" clipboard.deleteMatching              - Delete unpinned entries matching a filter (params: query?, entryType?)")
		...
		log.Info(" clipboard.search                      - Search history (params: query?, mimeType?, entryType?, isImage?, pinned?, limit?, offset?, beforeId?, before?, after?)")
