...
func (s *Server) Serve(printDocs bool) error {
	...
	if printDocs {
		...
		log.Info(" clipboard.deleteMatching                - Delete unpinned entries matching search (params: query?, entryType?)")
		...
		log.Info(" clipboard.search                      - Search history (params: query?, entryType?, pinned?, limit?, beforeId?)")
